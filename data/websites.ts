...
export function pushWebsitesToServer(): void {
  try {
    ...
    fetch('/api/websites', {
      ...
      .catch(e => {
        clearTimeout(timer);
        console.warn('Notice: Background sync of websites to server postponed (offline/slow):', e?.message || e);
      });
  } catch (e) {
    console.warn('pushWebsitesToServer offline notice:', e);
  }
}
...
export async function syncSharedWebsites(): Promise<WebsiteInfo[]> {
  try {
    ...
    const res = await fetch('/api/websites', {
      ...
      clearTimeout(timer);
      console.warn('Backend server website sync offline, using local website cache:', err?.message || err);
      return null;
    });
    ...
    console.warn('Notice: Backend website sync offline, continuing with local website cache:', err?.message || err);
  }
