...
import { searchCustomerLocal, saveOrReplaceCustomerRecordLocal, deleteCustomerRecordLocal, formatDateDDMMMYYYY, formatDateYYYYMMDD, fetchSharedRecordsAsync, displayVal } from '../utils/gasHelper';
...
export const CustomerSearch: React.FC<CustomerSearchProps> = ({
  ...
  const handleOpenAddIdModal = (rec: CustomerRecord) => {
    setAddIdRecord(rec);
    setNewCustomerId(rec.customerId && rec.customerId !== 'N/A' && rec.customerId !== '-' ? rec.customerId : '');
    setNewPassword(rec.password && rec.password !== 'N/A' && rec.password !== '-' ? rec.password : '');
    ...
    setAddIdBankAccountName(rec.bankAccountName && rec.bankAccountName !== 'N/A' && rec.bankAccountName !== '-' ? rec.bankAccountName : '');
    setAddIdBankAccountNumber(rec.bankAccountNumber && rec.bankAccountNumber !== 'N/A' && rec.bankAccountNumber !== '-' ? rec.bankAccountNumber : '');
  ...
  const handleOpenAddDepositModal = (rec: CustomerRecord) => {
    ...
    setBankAccountName(rec.bankAccountName && rec.bankAccountName !== 'N/A' && rec.bankAccountName !== '-' ? rec.bankAccountName : '');
    setBankAccountNumber(rec.bankAccountNumber && rec.bankAccountNumber !== 'N/A' && rec.bankAccountNumber !== '-' ? rec.bankAccountNumber : '');
  };
  ...
  const handleAddIdSubmit = (e: React.FormEvent) => {
    ...
    saveOrReplaceCustomerRecordLocal(addIdRecord, 'ADD_ID', {
      ...
      bankAccountName: addIdBankAccountName.trim() || '-',
      bankAccountNumber: addIdBankAccountNumber.trim() || '-',
      csId: addIdCsId
    });
  ...
  const handleAddDepositSubmit = (e: React.FormEvent) => {
    ...
    const msg = isTodayRecord
      ? `បាន Deposit  $${parsedAmt} រួចរាល់! ទិន្នន័យថ្ងៃនេះរបស់ ${addDepositRecord.profileName} ត្រូវបានលុបជំនួសដោយទិន្នន័យថ្មីដោយជោគជ័យ។`
      : `បានបង្កើតកំណត់ត្រា Deposit  $${parsedAmt} សម្រាប់ថ្ងៃនេះរួចរាល់! (ទិន្នន័យកាលពីថ្ងៃ ${addDepositRecord.regDate} ត្រូវបានរក្សាទុក)`;
  ...
  const handleConfirmDelete = () => {
    ...
    deleteCustomerRecordLocal(deleteConfirmRecord.id, deleteConfirmRecord);
    setAlertMessage(`បានលុបទិន្នន័យរបស់ ${deleteConfirmRecord.profileName} និងជួរដេកក្នុង Google Sheet រួចរាល់ដោយជោគជ័យ!`);
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="p-6 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold text-white flex items-center gap-2">
              ...
              <span>{t(activeLang, 'quickCustomerSearch')}</span>
            </h2>
            <p className="text-xs text-slate-400 mt-0.5">
              {t(activeLang, 'searchHintDetailed')}
            </p>
          </div>
          ...
          <div className="flex items-center bg-slate-800 p-1 rounded-xl border border-slate-700/80">
            ...
            >
              {t(activeLang, 'websiteLabel')} {currentWebsite}
            </button>
            <button
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={t(activeLang, 'searchByPlaceholder')}
      ...
      {query && (
        <div className="p-4 bg-slate-900/90 border border-slate-800 rounded-xl flex items-center justify-between text-xs text-slate-300">
          <div className="flex items-center gap-2">
            ...
            <span>{t(activeLang, 'recordsFound')} <strong className="text-white">{results.records.length}</strong> {t(activeLang, 'recordsCount')}</span>
          </div>
          <div className="text-emerald-400 font-bold text-sm">
            {t(activeLang, 'totalDepositAmount')}: ${results.totalDeposit.toLocaleString('en-US', { minimumFractionDigits: 2 })}
          </div>
        </div>
      )}
          ...
          return (
            ...
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                <div>
                  <span className="text-slate-500 block"> ID Customer</span>
                  <span className="text-slate-200 font-mono font-bold">{displayVal(rec.customerId)}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">Password</span>
                  <span className="text-slate-200 font-mono">{displayVal(rec.password)}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">Platform</span>
                  <span className="text-slate-200 font-medium">{displayVal(rec.platform)} ({displayVal(rec.sourceName)})</span>
                </div>
                <div>
                  <span className="text-slate-500 block">តំណភ្ជាប់ / Phone</span>
                  <span className="text-blue-400 font-medium truncate block">{displayVal(rec.contactLink)}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">ធនាគារ</span>
                  <span className="text-slate-200 font-medium">{displayVal(rec.bankName)}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">ឈ្មោះគណនី</span>
                  <span className="text-slate-200 font-medium uppercase">{displayVal(rec.bankAccountName)}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">លេខគណនី</span>
                  <span className="text-slate-200 font-mono">{displayVal(rec.bankAccountNumber)}</span>
                </div>
                <div>
      ...
      {addIdRecord && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 backdrop-blur-md overflow-y-auto animate-fade-in">
          <div className="relative w-full max-w-xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-4 sm:p-6 space-y-4 max-h-[90vh] overflow-y-auto touch-pan-y overscroll-contain my-auto">
              ...
              <div className="p-3.5 bg-slate-950 border border-slate-800 rounded-xl space-y-2 text-xs">
                ...
                <div className="grid grid-cols-2 gap-2 text-slate-300">
                  ...
                  <div>
                    ...
                    <strong className="text-blue-400">{displayVal(addIdRecord.contactLink)}</strong>
                  </div>
                  <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-200 mb-1.5">
                    Deposit Amount <span className="text-slate-400 font-normal"> </span>
                  </label>
                  <div className="relative">
          ...
          <div className="relative w-full max-w-xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-4 sm:p-6 space-y-4 max-h-[90vh] overflow-y-auto touch-pan-y overscroll-contain my-auto">
            ...
            <form onSubmit={handleAddDepositSubmit} className="space-y-4">
              ...
              <div className="pt-3 border-t border-slate-800 flex items-center justify-end gap-3">
                ...
                >
                  <CheckCircle2 className="w-4 h-4" />
                  <span>រក្សាទុក Deposit (Submit & Replace)</span>
                </button>
              </div>
            </form>
          </div>
      ...
      {deleteConfirmRecord && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 backdrop-blur-md overflow-y-auto animate-fade-in">
          <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-4 sm:p-6 space-y-4 max-h-[90vh] overflow-y-auto touch-pan-y overscroll-contain my-auto">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <div className="flex items-center gap-3">
