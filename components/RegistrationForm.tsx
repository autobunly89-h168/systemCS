...
import { QrBankingScanner } from './QrBankingScanner';
...
export const RegistrationForm: React.FC<RegistrationFormProps> = ({
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('websitesUpdated', handleWebsitesUpdated);
    };
  }, []);
  ...
  const handleRemoveCustomPlatform = (pToRemove: string) => {
    const updated = customPlatforms.filter((p) => p !== pToRemove);
    setCustomPlatforms(updated);
    localStorage.setItem('cs_custom_platforms', JSON.stringify(updated));
    if (platform === pToRemove) setPlatform('' as PlatformType);
  };
  ...
  const handleRemoveCustomBank = (bToRemove: string) => {
    const updated = customBanks.filter((b) => b !== bToRemove);
    setCustomBanks(updated);
    localStorage.setItem('cs_custom_banks', JSON.stringify(updated));
    if (bankName === bToRemove) setBankName('');
  };

  const handleQrBankScanSuccess = (data: {
    bankName: string;
    accountName: string;
    ...
    if (data.bankName) {
      ...
      if (!BANK_OPTIONS.includes(data.bankName) && !customBanks.includes(data.bankName)) {
        const updated = [...customBanks, data.bankName];
        setCustomBanks(updated);
        localStorage.setItem('cs_custom_banks', JSON.stringify(updated));
      }
    }
    if (data.accountName) {
      setBankAccountName(data.accountName.toUpperCase());
    }
    if (data.accountNumber) {
      setBankAccountNumber(data.accountNumber);
    }
    if (data.amount && (!depositAmount || depositAmount === '' || depositAmount === '0')) {
  ...
  useEffect(() => {
    if (activeCsId) {
      setCsId(activeCsId === 'ALL' ? 'CS-01' : activeCsId);
    }
  }, [activeCsId]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-xl font-bold text-white mt-1">
              {isDepositOnly ? t(activeLang, 'depositForm') : t(activeLang, 'newRegisterForm')}
            </h2>
            <p className="text-xs text-slate-400">
              {currentWebsite} - Register daily
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-amber-300">
                {t(activeLang, 'duplicateWarning')} ({duplicateWarning.matchType})
              </h4>
      ...
      <form onSubmit={(e) => handleSubmit(e, false)} className="p-6 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl space-y-6">
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wider mb-4 flex items-center gap-2 border-b border-slate-800 pb-2">
            ...
            <span>{t(activeLang, 'customerInfo')}</span>
          </h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1.5 flex items-center justify-between">
                <span>{t(activeLang, 'customerIdLabel')}</span>
              </label>
              <input
                type="text"
                value={customerId}
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-semibold text-slate-300">
                  {t(activeLang, 'passwordLabel')}
                </label>
                <button
                  type="button"
                  onClick={handleGeneratePassword}
                  className="text-[11px] text-blue-400 hover:underline flex items-center gap-1 font-medium cursor-pointer"
                >
            ...
            <div>
              ...
              <div className="relative">
                <input
                  type="text"
                  list="platform-form-options"
                  value={platform}
                  onChange={(e) => setPlatform(e.target.value as PlatformType)}
                  placeholder="ជ្រើសរើស ឬ វាយបញ្ចូល Platform..."
                  className="w-full pl-3.5 pr-8 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium"
                />
                {platform && (
                  <button
                    type="button"
                    onClick={() => setPlatform('' as PlatformType)}
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-rose-400 transition cursor-pointer"
                    title="លុប/សម្អាត (Clear)"
                  >
                    <X className="w-4 h-4" />
                  </button>
                )}
              </div>
              <datalist id="platform-form-options">
            ...
            <div className="relative" ref={sourceDropdownRef}>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-semibold text-slate-300">
                  Source Name <span className="text-slate-500 font-normal"></span>
                </label>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
              ...
              <div className="relative">
                <input
                  ...
                  placeholder=""
                  className="w-full pl-3.5 pr-14 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                Phone,username,link FB,Link Bot
              </label>
              <input
                type="text"
                value={contactLink}
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-2">
            ...
            <span className="text-[11px] text-emerald-400/90 font-medium flex items-center gap-1">
              ...
              <span>អាចស្គេន ឬ Paste រូប QR Bank បំពេញស្វ័យប្រវត្តិ</span>
            </span>
          </div>

          {/* QR Banking Scanner */}
          <QrBankingScanner onScanSuccess={handleQrBankScanSuccess} />

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-1">
            {/* Deposit Amount */}
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                Deposit Amount <span className="text-slate-500 font-normal">(-)</span>
              </label>
              <div className="relative">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  type="text"
                  list="bank-name-form-options"
                  value={bankName}
                  onChange={(e) => setBankName(e.target.value)}
                  placeholder="ជ្រើសរើស ឬ វាយបញ្ចូលឈ្មោះធនាគារ..."
                  className="w-full pl-3.5 pr-8 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium"
                />
                {bankName && (
                  <button
              ...
              <datalist id="bank-name-form-options">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button
                  type="button"
                  onClick={() => setCsId(activeCsId === 'ALL' ? 'CS-01' : activeCsId)}
                  className="text-[11px] text-blue-400 hover:text-blue-300 font-medium cursor-pointer flex items-center gap-1 bg-blue-950/80 hover:bg-blue-900/80 px-2 py-0.5 rounded-lg border border-blue-500/30 transition"
                  title="ទាញស្វ័យប្រវត្តិពី Header"
                >
                  <RefreshCw className="w-3 h-3" />
                  <span>ទាញពី Header ({activeCsId === 'ALL' ? 'CS-01' : activeCsId})</span>
                </button>
              </div>
              <input
                type="text"
      ...
      {showAddPlatformModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-slate-900 border border-slate-700/80 rounded-2xl max-w-sm w-full p-5 shadow-2xl space-y-4">
            ...
            {/* Added custom platforms list */}
            {customPlatforms.length > 0 && (
              <div className="pt-2 border-t border-slate-800 space-y-1.5">
                <div className="text-[11px] font-semibold text-slate-400 flex items-center justify-between">
                  <span>បញ្ជី Platform ដែលបានថែម ({customPlatforms.length}):</span>
                  <span className="text-[10px] text-slate-500">ចុច × ដើម្បីលុប</span>
                </div>
                <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto p-1.5 bg-slate-950/50 rounded-xl border border-slate-800">
                  {customPlatforms.map((p) => (
                    <span
                      key={p}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-800 border border-slate-700 rounded-lg text-xs font-medium text-slate-200"
                    >
                      <span>{p}</span>
                      <button
                        type="button"
                        onClick={() => handleRemoveCustomPlatform(p)}
                        className="p-0.5 text-slate-400 hover:text-rose-400 hover:bg-rose-500/20 rounded transition cursor-pointer"
                        title={`លុប ${p}`}
                      >
                        <X className="w-3 h-3" />
                      </button>
      ...
      {showAddSourceModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-slate-900 border border-slate-700/80 rounded-2xl max-w-sm w-full p-5 shadow-2xl space-y-4">
            ...
            {/* Added custom sources list */}
            {pasteHistory.length > 0 && (
              <div className="pt-2 border-t border-slate-800 space-y-1.5">
                <div className="text-[11px] font-semibold text-slate-400 flex items-center justify-between">
                  <span>បញ្ជី Source Name ក្នុងប្រវត្តិ ({pasteHistory.length}):</span>
                  <span className="text-[10px] text-slate-500">ចុច × ដើម្បីលុប</span>
                </div>
                <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto p-1.5 bg-slate-950/50 rounded-xl border border-slate-800">
                  {pasteHistory.map((s) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => removePasteHistoryItem(s)}
                        className="p-0.5 text-slate-400 hover:text-rose-400 hover:bg-rose-500/20 rounded transition cursor-pointer"
                        title={`លុប ${s}`}
                      >
                        <X className="w-3 h-3" />
      ...
      {showAddBankModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-slate-900 border border-slate-700/80 rounded-2xl max-w-sm w-full p-5 shadow-2xl space-y-4">
            ...
            {/* Added custom banks list */}
            {customBanks.length > 0 && (
              <div className="pt-2 border-t border-slate-800 space-y-1.5">
                <div className="text-[11px] font-semibold text-slate-400 flex items-center justify-between">
                  <span>បញ្ជីធនាគារដែលបានថែម ({customBanks.length}):</span>
                  <span className="text-[10px] text-slate-500">ចុច × ដើម្បីលុប</span>
                </div>
                <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto p-1.5 bg-slate-950/50 rounded-xl border border-slate-800">
                  {customBanks.map((b) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => handleRemoveCustomBank(b)}
                        className="p-0.5 text-slate-400 hover:text-rose-400 hover:bg-rose-500/20 rounded transition cursor-pointer"
                        title={`លុប ${b}`}
                      >
                        <X className="w-3 h-3" />
