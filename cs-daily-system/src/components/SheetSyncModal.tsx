...
import { getSavedWebAppUrl, saveWebAppUrl, syncToGoogleSheetApi, loadLocalRecords } from '../utils/gasHelper';
import { getWebsiteToken } from '../data/websites';
import { FileSpreadsheet, Key, Link as LinkIcon, Check, Copy, AlertCircle, RefreshCw, Send, ShieldCheck, Sparkles, X, Info } from 'lucide-react';
...
export const SheetSyncModal: React.FC<SheetSyncModalProps> = ({
  ...
  const [webAppUrl, setWebAppUrl] = useState<string>(() => getSavedWebAppUrl());
        ...
        <div className="p-5 sm:p-6 space-y-5 overflow-y-auto max-h-[75vh]">
            ...
            <div className="flex items-center justify-between text-xs pt-1 border-t border-slate-700/60">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span>{copiedToken ? 'បានចម្លង!' : 'Copy'}</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 bg-slate-950/60 border border-slate-800 p-4 rounded-xl">
            ...
            <button
              ...
              disabled={isSyncing}
              className="w-full py-3 bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 text-slate-950 font-bold text-xs rounded-xl shadow-lg transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
            >
              {isSyncing ? (
                <>
                  <RefreshCw className="w-4 h-4 animate-spin text-slate-950" />
                  <span>កំពុងបញ្ជូនទិន្នន័យទៅ Google Sheet...</span>
                </>
              ) : (
                <>
                  <Send className="w-4 h-4 text-slate-950" />
                  <span>បញ្ជូនទិន្នន័យទៅ GOOGLE SHEET ឥឡូវនេះ (Sync All Data)</span>
                </>
              )}
            </button>
