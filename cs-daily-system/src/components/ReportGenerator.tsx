...
export const ReportGenerator: React.FC<ReportGeneratorProps> = ({ currentWebsite, lang = 'km' }) => {
  ...
  const filtered = allRecords.filter(r => {
    ...
    const recDate = r.depositDate || r.regDate;
    const isAfterStart = !startDate || recDate >= startDate;
    const isBeforeEnd = !endDate || recDate <= endDate;

    return matchWeb && matchPlatform && matchSrc && isAfterStart && isBeforeEnd;
  });

  const totalRegistered = filtered.filter(r => r.status === 'New Register' || !r.status.includes('Deposit')).length;
  const totalDeposits = filtered.filter(r => r.depositAmount > 0).length;
  const totalAmount = filtered.reduce((sum, r) => sum + (Number(r.depositAmount) || 0), 0);
  const totalComIn = filtered.length + Math.round(filtered.length * 0.4);
    ...
    <div className="space-y-6 animate-fade-in max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-white flex items-center gap-2">
              ...
              <span>របាយការណ៍សកម្មភាព CS (Performance Reports)</span>
            </h2>
            <p className="text-xs text-slate-400 mt-0.5">
              ចម្រាញ់របាយការណ៍តាមកាលបរិច្ឆេទ វេបសាយ ប្រព័ន្ធ និង ប្រភពអតិថិជន
            </p>
          </div>
          ...
          >
            ...
            <span>ទាញយករក្សាទុក Report (CSV)</span>
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 bg-slate-900 border border-slate-800 rounded-2xl">
          <span className="text-xs font-bold text-slate-400 uppercase">ComIn សរុប</span>
          <div className="text-2xl font-extrabold text-white mt-2">{totalComIn}</div>
        </div>
        <div className="p-5 bg-slate-900 border border-slate-800 rounded-2xl">
          <span className="text-xs font-bold text-slate-400 uppercase">បានចុះឈ្មោះសរុប</span>
          <div className="text-2xl font-extrabold text-white mt-2">{totalRegistered}</div>
        </div>
        <div className="p-5 bg-slate-900 border border-slate-800 rounded-2xl">
          <span className="text-xs font-bold text-slate-400 uppercase">ប្រតិបត្តិការប្រាក់បញ្ញើ</span>
        ...
        <div className="p-5 bg-gradient-to-br from-slate-900 via-emerald-950/30 to-slate-900 border border-emerald-500/30 rounded-2xl">
          <span className="text-xs font-bold text-emerald-400 uppercase">ទឹកប្រាក់សរុប</span>
